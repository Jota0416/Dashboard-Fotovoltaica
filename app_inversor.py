...
def renderizar_aba_mensal(df_mes):
    ...
    st.plotly_chart(px.imshow(df_pivot, aspect="auto", color_continuous_scale="Viridis"), use_container_width=True)

# --- FUNÇÕES DIÁRIAS ---
def renderizar_aba_curvas(df):
    df_media = df.groupby('Tempo')['Valor'].mean().reset_index()
    st.plotly_chart(px.line(df, x='Tempo', y='Valor', color='Nome do data point', title="Curvas de Corrente"), use_container_width=True)
    st.divider()
...
if arquivo:
    ...
    if df_full is not None:
        ...
        if sel_str:
            ...
            with t[2]: st.plotly_chart(px.box(df_fd, x='Nome do data point', y='Valor', color='Nome do data point', title="Dispersão Diária"), use_container_width=True)
            with t[3]: st.plotly_chart(px.bar(df_fd.groupby('Nome do data point')['Valor'].sum().reset_index(), x='Nome do data point', y='Valor', color='Nome do data point', title="Acumulado Diário"), use_container_width=True)
            with t[4]: st.plotly_chart(px.imshow(df_fd.pivot_table(index='Nome do data point', columns='Hora', values='Valor', aggfunc='mean'), aspect="auto", color_continuous_scale="Viridis", title="Heatmap Diário"), use_container_width=True)
            with t[5]: 
                df_a = df_fd[df_fd['Valor'] >= 0.5]
                if not df_a.empty:
                    res = df_a.groupby('Nome do data point').agg(Inicio=('Tempo', 'min'), Fim=('Tempo', 'max')).reset_index()
                    st.plotly_chart(px.timeline(res, x_start="Inicio", x_end="Fim", y="Nome do data point", color="Nome do data point", title="Período Ativo"), use_container_width=True)
            with t[6]:
